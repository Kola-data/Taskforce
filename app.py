...
app.config['MAIL_DEFAULT_SENDER'] = 'Alert-info@demomailtrap.com'
mail = Mail(app)
mysql = MySQL(app)
...
def index():
    ...
    net_balance = total_budget - total_income
    ...
    return render_template('index.html', transactions=formatted_transactions, total_income=total_income,
                           total_expense=total_expense, net_balance=net_balance)
...
def view_users():
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM users WHERE type!=3")
...
def add_user():
    if request.method == 'POST':
        ...
        try:
            ...
            # Handle duplicate entry for email or username
...
def edit_user(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute("""
                UPDATE users 
                SET full_name = %s, username = %s, email = %s, password = %s, type = %s 
                WHERE id = %s
            """, (fullname, username, email, password, notify, id))
            mysql.connection.commit()
...
def delete_user(id):
    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE users SET type = %s WHERE id = %s", (3, id))
    mysql.connection.commit()
    cursor.close()
...
def categories():
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM categories WHERE status != %s", (3,))
...
def delete_category(id):
    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE categories SET status = %s WHERE id = %s", (3, id,))
    mysql.connection.commit()

    cursor.execute("UPDATE sub_categories SET status = %s WHERE cat_id = %s", (3, id,))
    mysql.connection.commit()
...
def sub_categories():
    ...
    query = """
        ...
        JOIN categories AS c ON sc.cat_id = c.id WHERE c.status != 3 and sc.status != 3
    """
...
def accounts():
    ...
    # Fetch all accounts
    cursor.execute("SELECT * FROM accounts WHERE status != %s", (3,))
...
def delete_account(id):
    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE accounts SET status = %s WHERE id = %s", (3, id))
    mysql.connection.commit()
    cursor.close()
